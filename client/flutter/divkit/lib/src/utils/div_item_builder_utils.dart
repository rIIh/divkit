  ...
  for (final item in items) {
    final selectors = [
      for (final prototype in builder.prototypes)
        [prototype, prototype.selector.requireValue],
    ];
    ...
    final id = selected.id?.requireValue;
...
Future<List<DivItemBuilderResult>> buildChildrenAsync({
  required DivCollectionItemBuilder builder,
  required DivVariableContext context,
}) async {
  final items = await builder.data.resolveValue(context: context);
  ...
  final results = await Future.wait(
    items.mapIndexed(
      (index, item) async {
        ...
        final selectors = await Future.wait([
          for (final prototype in builder.prototypes)
            exprResolver
                .resolve(prototype.selector, context: itemContext)
                .then((value) => [prototype, value]),
        ]);
        ...
        final id = await selected.id?.resolveValue(context: itemContext);
        final Div result = selected.div.copy(id: id);
